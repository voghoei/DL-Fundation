...
def get_clean_data():
    ...
    for i in range(num_train):
        if y_train[i][0] == 1:
            y_train_encode.append(0)
        else:
            y_train_encode.append(1)

    for i in range(num_val):
        if y_val[i][0] == 1:
            y_val_encode.append(0)
        else:
            y_val_encode.append(1)

    for i in range(num_test):
        if y_test[i][0] == 1:
            y_test_encode.append(0)
        else:
            y_test_encode.append(1)
...
def generate_word_count(clean_data):
    ...
    pickle.dump([X_train_count, X_val_count, X_test_count, y_train_encode, y_val_encode, y_test_encode], open("../../data/word_count_2class.p", "wb"))
...
def generate_TFIDF(clean_data):
    ...
    pickle.dump([X_train_tfidf, X_val_tfidf, X_test_tfidf, y_train_encode, y_val_encode, y_test_encode], open("../../data/tfidf_2class.p", "wb"))
...
def generate_ngram(clean_data):
    ...
    pickle.dump([X_train_tfidf_ngram, X_val_tfidf_ngram, X_test_tfidf_ngram, y_train_encode, y_val_encode, y_test_encode], open("../../data/tfidf_ngram_2class.p", "wb"))
